...
// Define a type for the job keys
type JobKeys = keyof typeof mockJobs;
...
export default function JobDetails({ params }: { params: { id: string } }) {
  ...
  const [job, setJob] = useState<Job | null>(null) // Allow job to be of type Job or null
  const [relatedJobs, setRelatedJobs] = useState<Job[]>([]) // Specify the type as Job[]

  useEffect(() => {
    const fetchedJob = mockJobs[params.id as JobKeys]
    if (fetchedJob) {
      setJob(fetchedJob)
      const fetchedRelatedJobs = Object.values(mockJobs)
        .filter(relatedJob => relatedJob.id !== params.id)
        .slice(0, 2)
      setRelatedJobs(fetchedRelatedJobs) // Now this will match the Job[] type
    } else {
  ...
  const handleShare = () => {
    ...
    alert(`Sharing job: ${job?.title} @ ${job?.company} with salary of ${job?.salary}`)
  }
  ...
  return (
    <div className="relative flex flex-col md:flex-row gap-8">
      ...
      <motion.div
        ...
        className="md:w-3/4 relative z-10" // Ensure content is above the lines
      >
